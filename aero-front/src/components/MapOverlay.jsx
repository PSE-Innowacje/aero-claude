...
/**
 * SVG overlay rysujący trasę lotu z gradientem PSE i punktami nawigacyjnymi.
 * Współdzielony między TrasaMapWidget (read-only) i TrasyLotowPage (edytor).
 *
 * @param {string} idPrefix    - prefiks id SVG (unikalne per instancja, np. "tw" / "tl")
 * @param {Array}  polyPixels  - tablica {x, y} pikseli po projekcji
 * @param {Array}  points      - tablica {lat, lng} surowych punktów (do obliczenia t gradientu)
 * @param {number} width       - szerokość SVG
 * @param {number} height      - wysokość SVG
 * @param {number|null} hoveredIdx - indeks podświetlonego punktu
 * @param {function} onHover   - callback(idx | null)
 * @param {boolean} interactive - czy punkty reagują na hover (default true)
 * @param {React.ReactNode} extraDots - opcjonalne dodatkowe elementy per punkt (np. numer w edytorze)
 */
      ...
      {polyPixels.map((p, i) => {
        ...
        return (
          ...
          >
            ...
            {/* Extra content per dot (np. numer punktu w edytorze) */}

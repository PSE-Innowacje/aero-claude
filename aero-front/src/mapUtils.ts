...
export function latLngToWorld(lat: number, lng: number, zoom: number): WorldPoint {
...
export function worldToLatLng(wx: number, wy: number, zoom: number): LatLng {
  const n = Math.pow(2, zoom) * 256;
...
export function latLngToPixel(
  ...
  return { x: p.x - c.x + mapW / 2, y: p.y - c.y + mapH / 2 };
}
...
export function haversine(p1: LatLng, p2: LatLng): number {
  const R = 6371;

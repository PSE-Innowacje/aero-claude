...
import {
  ...
  message, Skeleton, DatePicker, Divider, Space, Typography,
} from 'antd';
...
import { radii, gradients, palette } from '../theme';
import type { SlownikDto, UzytkownikDto, OperacjaPayload } from '../types/api';
...
export default function OperacjaFormPage() {
  ...
  const [rodzaje, setRodzaje]         = useState<SlownikDto[]>([]);
  const [uzytkownicy, setUzytkownicy] = useState<UzytkownikDto[]>([]);
  const [kmlPlik, setKmlPlik]         = useState<KmlFile | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [r, u, data] = await Promise.all([
          getRodzajeCzynnosci(),
          getUzytkownicyKontakty(signal),
          isEdit ? getOperacjaById(Number(id), signal) : Promise.resolve(null),
        ]);
        ...
        if (!signal.aborted) {
          message.error(extractApiError(err, 'Nie udało się pobrać danych.'));
        }
      } finally {
  ...
  const onFinish = async (values: OperacjaFormValues) => {
    ...
    try {
      if (isEdit) {
        await updateOperacja(Number(id), payload);
  ...
  const gradient = isEdit ? gradients.formEdit : gradients.formNew;
      ...
      <Card style={{ borderRadius: radii.xxl }} styles={{ body: { padding: 32 } }}>
        {initLoad ? <Skeleton active paragraph={{ rows: 8 }} /> : (
          <Form form={form} layout="vertical" onFinish={onFinish} requiredMark="optional">
            ...
            <div style={{ background: 'rgba(108,71,255,0.07)', borderRadius: radii.lg, padding: '14px 18px', marginBottom: 24, border: '1px solid rgba(108,71,255,0.2)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                {kmlPlik ? (
                  <div style={{
                    ...
                    <FileOutlined style={{ color: palette.success, fontSize: 16 }} />
                    <Text style={{ flex: 1, color: palette.successText, fontWeight: 500, fontSize: 13 }}>
                      {kmlPlik.nazwa}
                    </Text>
                    <Button type="text" danger size="small" icon={<DeleteOutlined />} onClick={usunKmlPlik} title="Usuń plik" />
                  </div>
                ) : (
                  <Text type="secondary" style={{ fontSize: 12 }}>Brak wybranego pliku. Akceptowane formaty: .kml, .xml</Text>
                )}
              </div>
            </Form.Item>

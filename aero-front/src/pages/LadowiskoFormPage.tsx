...
export default function LadowiskoFormPage() {
  ...
  const onFinish = async (values: Record<string, unknown>) => {
    setLoading(true);
    try {
      isEdit ? await updateLadowisko(Number(id), values) : await createLadowisko(values);
  ...
  return (
    <div style={{ maxWidth: 540, margin: '0 auto' }}>
      <PageHeader
        icon={<EnvironmentOutlined style={{ color: '#fff', fontSize: 20 }} />}
        gradient="linear-gradient(135deg, #fa709a 0%, #fee140 100%)"
        title={isEdit ? 'Edytuj lądowisko' : 'Nowe lądowisko'}
        backTo="/ladowiska"
      />
        ...
        {initLoad ? <Skeleton active paragraph={{ rows: 4 }} /> : (
          <Form form={form} layout="vertical" onFinish={onFinish} requiredMark="optional">
            ...
            <Form.Item label="Opis" name="opis">
              <Input.TextArea rows={2} placeholder="Opcjonalny opis" />
            </Form.Item>
